...
import {parse_route_id} from '../../../../node_modules/@sveltejs/kit/src/utils/routing';
...
modules.forEach((module) => {
    ...
    module.types.forEach((type) => {
        const link = `/types#${slug}-${slugify(type.name)}`;
...
function parse({ file, body, code, codespan }) {
    ...
    const content = transform(body, {
        ...
        heading(html, level) {
            ...
            headings[level - 1] = slugify(title);
            ...
            if (generics !== undefined) {
                return [
                    ...
                    generics
                        ...
                        .replace('  }>', '}>') // unindented closing brace
                ];
            }
        }
        return [name];
